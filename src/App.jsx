  ...
  return (
    <div className="form" onSubmit={handleSubmit}>
      ...
      <p className="form-sub">
        Share enough that our team understands your situation. Your story and contact info stay private.
      </p>
      ...
      <label className="field">
        ...
        <textarea rows={5} value={form.story} onChange={(e) => update("story", e.target.value)} />
      </label>
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="form-sub" style={{ marginTop: "12px", fontSize: "0.8rem" }}>
          Instagram and TikTok don't let other sites post directly with a link. Copy the caption above and paste it when you post.
        </p>
...
export default function App() {
  ...
  async function loadRequests() {
    ...
    if (isAdmin) {
      ...
      if (msgs) {
        const counts = {};
        msgs.filter((m) => !m.fromAdmin).forEach((m) => {
  ...
  return (
    <div className="app">
      <header className="header">
        <div className="header-inner">
          ...
          <div className="header-actions">
            {isAdmin ? (
              <button className="btn-ghost" onClick={() => setIsAdmin(false)}>
      ...
      <main className="main">
        ...
        {filtered.length === 0 && (
          <div className="empty">
            ...
            <button className="btn-primary" onClick={() => { setShowForm(true); setSubmitted(false); }}>
              Be the first to post <ArrowRight size={16} />
            </button>
          </div>
        )}
      ...
      {showAdminLogin && (
        <AdminLogin onLogin={() => { setIsAdmin(true); setShowAdminLogin(false); loadRequests(); }} />
      )}
      ...
      {threadReq && (
        <ThreadModal
          ...
          isAdmin={isAdmin}
          onClose={() => setThreadReq(null)}
          onSend={sendMessage}
        />
      )}

      {shareReq && (
        <ShareModal request={shareReq} onClose={() => setShareReq(null)} />
      )}
    </div>
  );
}
